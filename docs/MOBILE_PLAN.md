# ScriptumIQ — Mobile application plan (Phase 1)

Source: *Claude Code Input: ScriptumIQ Mobile App*, v1.0, 2026-09-13, 15 requirements, plus the
...
In the UI: ordinary editing is ordinary editing, and *Fix what ScriptumIQ read* is a distinct,
deliberate action on the decoded text — the same distinction the web's `EditableItem` tooltip
already gestures at ("Click to fix what ScriptumIQ read"), made explicit now that editing is the
...
`handoff.test.ts` asserts. Navigation inside the frame is held to the two ScriptumIQ hosts;
...
- and the CLAUDE.md amendment that says ScriptumIQ reads a tablet.
...
5. ~~**Editing**~~ — **done** (2026-09-14). Editors for actions, calendar entries and note
   ...
   two acts are two controls: Save is `items.update`, "Fix what ScriptumIQ read" is
...
**Windows MAX_PATH is why that script exists**, and it took three separate fixes. React Native
ships C++ sources with long names, CMake mangles each source's FULL path into its object filename,
and this repo sits under `OneDrive\Documents`. Any one of these alone leaves the build failing with
`clang ... is not able to compile a simple test program`, which reads like a broken NDK and is not:

1. **`nodeLinker: hoisted`** in `pnpm-workspace.yaml`. The decisive one. pnpm's default layout
   buries every package under `.pnpm/<name>@<version>_<32 hex>/`, and the overflow is in the
   SOURCE path, so no amount of relocating build output helps.
2. **`buildDir` and `.cxx` relocated to `C:\dmb`**, for every subproject. Codegen writes C++ under
   `buildDir`, so both halves of the object path have to shrink.
3. **Re-applying (2) on every run**, because `expo prebuild` regenerates `android/` and discards
   it. That cannot live in a committed file, which is the whole reason for the script.

The alternatives, if this ever stops holding: `LongPathsEnabled` in the registry plus a reboot, or
keeping the repo somewhere shorter. `eas.json` also has cloud profiles (`preview` for an APK,
`production` for the Play AAB) which sidestep the limit entirely by building on Linux.

**The splash is in two halves, and has to be.** Android 12+ masks the native splash icon to a
circle — the platform's API, not Expo's choice — so a wide lockup put there is cropped to a band
through its middle. The native half shows the emblem, which is circular and survives; the app then
draws the full lockup itself over the same Parchment ground, held three seconds. See
`app/_layout.tsx`. The hold covers the font load too: gating it on the fonts was a bug, because the
six brand faces take longer than three seconds and the timer expired against a blank screen.

**Before the first Play upload**, still missing: a feature graphic (1024x500), screenshots, and a
privacy policy URL — `/privacy` already exists on the public site. The icon and the 512x512 Play
listing icon are generated by `scripts/app-icons.py`.
...
6. **`pnpm db:dev` will not start on this machine.** PGlite 0.5.8 aborts opening the existing
   data directory under `%LOCALAPPDATA%\ScriptumIQ\pgdata` (`RuntimeError: Aborted()`), twice,
