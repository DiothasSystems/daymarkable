...
- TypeScript (strict), Node 22, pnpm workspaces monorepo:
  ...
  - `apps/web` — Next.js, fully responsive (mobile HTML experience). One server, two hosts
    ...
    (`src/components/hero/`), generated by `python scripts/hero-build-gen.py` from the Claude
    Design handoff in `design/design_handoff_hero_animation/` (logo build + looping
    tablet → machine → pages scene, pure CSS on inline SVG); edit the handoff and regenerate
    rather than hand-editing the SVG. `HeroScene` in the same folder is the earlier cinematic
