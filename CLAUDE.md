...
- TypeScript (strict), Node 22, pnpm workspaces monorepo:
  ...
  - `apps/web` — Next.js, fully responsive (mobile HTML experience): the public marketing
    ...
    (`src/components/hero/`) assembles the supplied artwork from cut-out layers generated by
    `python scripts/hero-slice.py` from `design/.../assets/hero-scene.png`; re-run the script
    if that artwork changes, never hand-edit `public/hero/`.
